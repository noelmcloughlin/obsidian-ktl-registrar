...
| `src/vocab.ts` | The pinned vocabulary manifest, the default settings it produces, and the saved-settings merge rule (import-free) |
...
- Run `npm run smoke-test` - the import-free modules must pass their fixture checks (no Obsidian install needed for this one; it runs under plain Node). If you have another real bundle to hand, point the suite at it too - `LOKF_EXTRA_BUNDLE=<path-to-a-bundle>/knowledge npm run smoke-test` - to catch an over-strict rule the in-repo fixtures wouldn't; it's opt-in precisely so the default suite stays hermetic.
- **`npm run smoke-test` covers only the import-free modules** (`validator.ts`, `locator.ts`, `vocab.ts`, `graph.ts`, `fixes.ts`, `propose.ts`, `report-filter.ts`, `suggest-context.ts`, `affordances.ts`, `fields.ts`). Anything that needs the Obsidian `App` - a vault scan's unreadable-file handling, bundle-root-folder resolution, the scaffold command's target-picking, the settings tab, the modals - has no automated test (there's no headless Obsidian to run one in) and must be checked by hand in a real vault first. That boundary is why logic worth testing is moved *out* of `main.ts` rather than tested through it: the saved-settings merge rule lives in `vocab.ts` as `mergeSavedSettings`, so `loadSettings` is a call and an assignment.
