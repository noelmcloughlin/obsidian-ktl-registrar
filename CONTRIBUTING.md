...
Thanks for your interest in improving KTL Registrar!
...
```bash
git clone https://github.com/noelmcloughlin/obsidian-ktl-registrar.git
cd obsidian-ktl-registrar
npm install
...
`main.js` is generated and git-ignored, so a fresh clone has none and Obsidian reports "Failed to load plugin" until you build. To test in a real vault, clone into `<your-vault>/.obsidian/plugins/ktl-registrar/`, or copy `main.js`, `manifest.json` and `styles.css` there, then reload Obsidian; the [Hot Reload](https://github.com/pjeby/hot-reload) plugin speeds up iteration. For a realistic bundle to test against, point a scratch vault at any project's `.lokf/knowledge/` directory.
...
- `src/lokf-vocab.json` is generated by `npm run build-vocab` from the schema the sidecar's pinned `lokf` ships (`.lokf/uv.lock`; needs `uv`). Refresh it, and commit the result, only when bumping that pin; it is kept identical to KTL Curator's.
