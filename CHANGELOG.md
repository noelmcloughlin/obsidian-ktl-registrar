...
- **Insert the bundle's semantic header** (was *Insert semantic header template into root index.md*): in a vault with no bundle it asks - *make this vault the exhibition* (the header goes on the root `index.md` and every note becomes a record; suggested when the vault holds nothing but an `index.md`, the empty vault someone just opened to be the bundle) or *create a `knowledge_bundle` folder inside it* (suggested otherwise; the notes around it are left alone) - instead of decorating the vault root unasked or always nesting a folder. The palette name drops its parenthetical.
